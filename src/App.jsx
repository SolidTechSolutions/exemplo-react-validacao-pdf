...
// Example front-end for validating signed PDF/PAdES documents.
//
// Two modes:
// - "backend" (default): talks to the local example backend
//   (exemplo-java-integracao-validacao-pdf, POST /api/pdf/validate/form). The
...
const DEFAULT_BACKEND_URL = 'http://localhost:8094';
    ...
    <div className="report">
      ...
      <table className="report-table">
        <tbody>
          ...
          <Row label="PDF/A profile" value={doc.pdfAProfile} />
        </tbody>
      </table>
...
export default function App() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (mode === 'backend') {
        ...
        url = `${backendUrl.replace(/\/$/, '')}/api/pdf/validate/form`;
      } else {
        documents.forEach((f, i) => fd.append(`signedFile[${i}]`, f));
        url = `${baseUrl.replace(/\/$/, '')}/solidsign/dsig/validation/verify-pdf`;
      }
  ...
  return (
    <div className="page">
      <h1>Validate PDF/PAdES (React example)</h1>
      <p className="subtitle">
        Example front-end for <code>exemplo-java-integracao-validacao-pdf</code>. By default it
      ...
      {docs.length > 0 && (
        <>
          ...
          <DocumentReport doc={docs[selectedIdx]} fileName={documents[selectedIdx]?.name || `document_${selectedIdx + 1}.pdf`} />
        </>
      )}
    </div>
  );
